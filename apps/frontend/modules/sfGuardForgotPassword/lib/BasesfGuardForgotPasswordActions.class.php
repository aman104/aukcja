  ...
  public function executeIndex($request)
  {
    ...
    if ($request->isMethod('post'))
    {
      ...
      if ($this->form->isValid())
      {
        ...
        Tools::sendEmail($this->form->user->email_address, 'Przypomnienie hasła', $this->getPartial('sfGuardForgotPassword/send_request', array('user' => $this->form->user, 'forgot_password' => $forgotPassword)));

        //$this->getMailer()->send($message);

        $this->getUser()->setFlash('notice', 'Odbierz pocztę');
  ...
  public function executeChange($request)
  {
    ...
    if ($request->isMethod('post'))
    {
      ...
      if ($this->form->isValid())
      {
        ...
        Tools::sendEmail($this->user->email_address, 'Hasło zostało zmienione', $this->getPartial('sfGuardForgotPassword/new_password', array('user' => $this->user, 'password' => $request['sf_guard_user']['password'])));

        //$this->getMailer()->send($message);

        $this->getUser()->setFlash('notice', 'Hasło zmienione poprawnie');
        $this->redirect('@sf_guard_signin');
      }
    }
  }
